...
function checkForBombs(x) {
    ...
    if (x[0] != 0){arr.push(boardArr[x[0]-1][x[1]])}
    if (x[0] != numberSquares-1) {arr.push(boardArr[x[0]+1][x[1]])}
    if (x[1] != 0){arr.push(boardArr[x[0]][x[1]-1])}
    if (x[1] != numberSquares - 1){arr.push(boardArr[x[0]][x[1]+1])}
    if (x[0] != 0 && x[1] != 0){arr.push(boardArr[x[0]-1][x[1]-1])}
    if (x[0] != numberSquares -1 && x[1] != numberSquares -1){arr.push(boardArr[x[0]+1][x[1]+1])}
    if (x[0] != 0 && x[1] != numberSquares -1) {arr.push(boardArr[x[0]-1][x[1]+1])}
    if (x[0] != numberSquares-1 && x[1] != 0) {arr.push(boardArr[x[0]+1][x[1]-1])}
...
board.addEventListener("contextmenu", x => {
   ...
   // x.target.innerText = "🚩"
    ...
    if (checkWin(boardArr)){lostGame("You found all the mines - Click to play again")}
